...
char odometer_str[16];

typedef union {
    uint32_t raw;
    struct {
        uint32_t num_presses : 32;
    };
} user_config_t;
...
void keyboard_post_init_user() {
    ...
    odometer = user_config.num_presses;
}

static void handle_odometer(void) {
    if (get_current_wpm() != 000) {
        saved = false;
    } else {
        if (!saved) {
            user_config.num_presses = odometer;
...
bool oled_task_user(void) {
    ...
    oled_set_cursor(0,0);
    sprintf(odometer_str, "ODO:%0lu", odometer);
    oled_write(odometer_str, false);
}
