...
static inline void init_row(void) {
    for (uint8_t i = 0; i < EC_MATRIX_ROWS; i++) {
...
void ecsm_config_init(void) {
    ...
    for (uint8_t i = 0; i < EC_MATRIX_ROWS; i++) {
        for (uint8_t j = 0; j < EC_MATRIX_COLS; j++) {
...
void ecsm_print_debug(void) {
    ...
    for (uint8_t i = 0; i < EC_MATRIX_ROWS+1; i++) {
        for (uint8_t j = 0; j < EC_MATRIX_COLS; j++) {
            if (!tuned) {
...
bool ecsm_matrix_scan(matrix_row_t current_matrix[]) {
    ...
    for (uint8_t col = 0; col < EC_MATRIX_COLS; col++) {
        for (uint8_t row = 0; row < EC_MATRIX_ROWS; row++) {
